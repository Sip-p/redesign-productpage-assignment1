...
const FAQ = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-indigo-50 to-purple-50 py-16 px-2 md:px-4 sm:px-6 lg:px-8">
      {/* Glowing Boxes */}
      <div className="absolute left-0 top-0 grid grid-cols-2 gap-2 z-0 h-full w-16">
        {Array.from({ length: 20 }).map((_, index) => (
          <div
            key={index}
            className={`w-full h-8 bg-blue-500 animate-pulse`}
            style={{
      ...
      <div className="absolute right-0 top-0 grid grid-cols-2 gap-2 z-0 h-full w-16">
        {Array.from({ length: 20 }).map((_, index) => (
          <div
            key={index}
            className={`w-full h-8 bg-blue-500 animate-pulse`}
